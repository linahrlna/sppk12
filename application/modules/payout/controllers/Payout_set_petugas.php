...
class Payout_set_petugas extends CI_Controller {
...
// Siswa
    ...
    $data['month'] = $this->Bulan_model_petugas->get_total($cashback);
    $data['beb'] = $this->Bebas_pay_model->get($cashback);
    $data['log'] = $this->Log_trx_model_petugas->get($logs);
    ...
    $data['main'] = 'payout/payout_list_petugas';
...
// Siswa
    ...
    $data['siswa'] = $this->Student_model_petugas->get(array('student_id'=>$siswa['student_id'], 'group'=>TRUE));
    $data['student'] = $this->Bulan_model_petugas->get($pay);
    $data['bulan'] = $this->Bulan_model_petugas->get($param);
    $data['bebas'] = $this->Bebas_model->get($pay);
    $data['free'] = $this->Bebas_pay_model->get($param);
    $data['s_bl'] = $this->Bulan_model_petugas->get_total($cashback);
    ...
    $html = $this->load->view('payout/payout_cetak_petugas_pdf', $data, true);
...
// Programs
    ...
    $data['bulan'] = $this->Bulan_model_petugas->get(array('id' => $id));
...
// View data detail
  ...
  public function payout_bulan($id = NULL, $student_id = NULL) {
    ...
    $data['in'] = $this->Bulan_model_petugas->get_total(array('status'=>1, 'payment_id' => $id, 'student_id' => $student_id));
    ...
    $data['title'] = 'Pembayaran Siswa';
    $data['main'] = 'payout/payout_add_bulan_petugas';
    $this->load->view('petugas/layout', $data);
  }
  ...
  public function payout_bebas($id = NULL, $student_id = NULL, $bebas_id = NULL, $pay_id =NULL) {
    ...
    if ($_POST == TRUE) {
      ...
      $lastletter = $this->Letter_model_petugas->get(array('limit' => 1));
      $student = $this->Bebas_model->get(array('id'=>$this->input->post('bebas_id')));
      $user = $this->Setting_model_petugas->get(array('id' => 8));
      $password = $this->Setting_model_petugas->get(array('id' => 9));
      ...
      if ($lastletter['letter_year'] < date('Y') OR count($lastletter) == 0) {
        ...
        $this->Letter_model_petugas->add(array('letter_number' => $nomor, 'letter_month' => date('m'), 'letter_year' => date('Y')));
        $nofull = date('Y'). date('m'). $nomor;
      }
  ...
  function pay($payment_id = NULL, $student_id =NULL, $id = NULL) { 
    ...
    $lastletter = $this->Letter_model_petugas->get(array('limit' => 1));
    $student = $this->Bulan_model_petugas->get(array('student_id'=>$student_id,'id'=>$id));
    $user = $this->Setting_model_petugas->get(array('id' => 8));
    $password = $this->Setting_model_petugas->get(array('id' => 9));
    $activated = $this->Setting_model_petugas->get(array('id' => 10));
  ...
  function not_pay($payment_id = NULL, $student_id =NULL, $id = NULL) { 
    ...
    $this->Log_trx_model_petugas->delete_log(array(
    ...
    $this->Bulan_model_petugas->add($pay);
    if ($this->input->is_ajax_request()) {
      echo $status;
    } else {
  ...
  function printPay($payment_id = NULL, $student_id =NULL, $id = NULL) {
    ...
    $html = $this->load->view('payout/payout_petugas_pdf', $data, true);
  ...
  function printPayFree($payment_id = NULL, $student_id =NULL, $id = NULL) {
    ...
    if ($id == NULL)
      redirect('petugas/payout/payout_bebas/'.$payment_id.'/'.$student_id);
    ...
    $html = $this->load->view('payout/payout_free_petugas_pdf', $data, true);
    $data = pdf_create($html, $data['printpay']['student_full_name'], TRUE, 'A4', TRUE);
  }
  ...
  function multiple() {
    ...
    if ($action == "printAll") {
      ...
      $data['printpay'] = $this->Bulan_model_petugas->get(array('multiple_id' => $print, 'group'=>TRUE));
      $data['pay'] = $this->Bulan_model_petugas->get(array('multiple_id' => $print));
      ...
      $html = $this->load->view('payout/payout_bulan_multiple_petugas_pdf', $data, true);
...
// Delete to database
  public function delete($id = NULL) {
    ...
    if ($_POST) {
      $this->Payment_model_petugas->delete($id);
// activity log
